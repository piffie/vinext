...
import { setHeadersContext, headersContextFromRequest, getDraftModeCookieHeader, getAndClearPendingCookies, consumeDynamicUsage, markDynamicUsage, getDynamicUsageCount, applyMiddlewareRequestHeaders, getHeadersContext, setHeadersAccessPhase } from "next/headers";
...
/**
 * ISR cache wrapper for route handlers. Centralizes cache read, dynamic
 * detection, cache write, and stale-while-revalidate background regen
 * into a single function. Uses getDynamicUsageCount() to bracket handler
 * execution, avoiding the fragile clear-before/read-after pattern.
 *
 * Returns { response, dynamicUsed } so the caller can still apply
 * Cache-Control headers and middleware transforms.
 */
async function __handleRouteWithIsrCache(opts) {
  const {
    pattern, routeKey, revalidateSeconds, isProduction,
    isForceDynamic, isGetOrHead, handlerFn, request, params,
    cleanPathname, url, execCtx, createRegenContext,
  } = opts;
  ...
  // ── 1. Cache read (skip for known-dynamic handlers) ──────────
  if (isIsrEligible && !__dynamicRouteHandlers.has(pattern)) {
    try {
      const cached = await __isrGet(routeKey);

      if (cached && !cached.isStale && cached.value.value && cached.value.value.kind === "APP_ROUTE") {
        // HIT
        const cv = cached.value.value;
        __isrDebug?.("HIT (route)", cleanPathname);
        const headers = Object.assign({}, cv.headers || {});
        headers["X-Vinext-Cache"] = "HIT";
        headers["Cache-Control"] = "s-maxage=" + revalidateSeconds + ", stale-while-revalidate";
        return {
          response: new Response(opts.isAutoHead ? null : cv.body, { status: cv.status, headers }),
          dynamicUsed: false,
          cacheStatus: "HIT",
        };
      }
      ...
      if (cached && cached.isStale && cached.value.value && cached.value.value.kind === "APP_ROUTE") {
        // STALE: serve stale, trigger background regen
        const sv = cached.value.value;
        const revalSecs = revalidateSeconds;
        const revalUrl = request.url;
        const revalSearchParams = new URLSearchParams(url.searchParams);
        const revalParams = params;

        __triggerBackgroundRegeneration(routeKey, async function() {
          const regenCtx = createRegenContext();
          await _runWithUnifiedCtx(regenCtx, async () => {
            _ensureFetchPatch();
            setNavigationContext({ pathname: cleanPathname, searchParams: revalSearchParams, params: revalParams });
            const syntheticReq = __proxyRouteRequest(
            ...
            const freshBody = await revalResponse.arrayBuffer();
            const freshHeaders = {};
            revalResponse.headers.forEach(function(v, k) {
              if (k !== "x-vinext-cache" && k !== "cache-control") freshHeaders[k] = v;
            });
            const routeTags = __pageCacheTags(cleanPathname, getCollectedFetchTags());
            await __isrSet(routeKey, { kind: "APP_ROUTE", body: freshBody, status: revalResponse.status, headers: freshHeaders }, revalSecs, routeTags);
            __isrDebug?.("route regen complete", routeKey);
          });
        });

        __isrDebug?.("STALE (route)", cleanPathname);
        const staleHeaders = Object.assign({}, sv.headers || {});
        staleHeaders["X-Vinext-Cache"] = "STALE";
        staleHeaders["Cache-Control"] = "s-maxage=0, stale-while-revalidate";
        return {
          response: new Response(opts.isAutoHead ? null : sv.body, { status: sv.status, headers: staleHeaders }),
          dynamicUsed: false,
          cacheStatus: "STALE",
        };
      }
    } catch (err) {
      console.error("[vinext] ISR route cache read error:", err);
    }
  }
  ...
  // ── 4. Apply ISR Cache-Control ──────────────────────────────
  if (revalidateSeconds !== null && !dynamicUsed && isGetOrHead && !handlerSetCacheControl) {
    response.headers.set("cache-control", "s-maxage=" + revalidateSeconds + ", stale-while-revalidate");
  }

  // ── 5. Cache write (MISS) ───────────────────────────────────
  if (isIsrEligible && !dynamicUsed && !handlerSetCacheControl) {
    response.headers.set("X-Vinext-Cache", "MISS");
    const routeClone = response.clone();
    const routeTags = __pageCacheTags(cleanPathname, getCollectedFetchTags());
    const writePromise = (async () => {
      try {
        const buf = await routeClone.arrayBuffer();
        const hdrs = {};
        routeClone.headers.forEach(function(v, k) {
          if (k !== "x-vinext-cache" && k !== "cache-control") hdrs[k] = v;
        });
        await __isrSet(routeKey, { kind: "APP_ROUTE", body: buf, status: routeClone.status, headers: hdrs }, revalidateSeconds, routeTags);
        __isrDebug?.("route cache written", routeKey);
      } catch (cacheErr) {
  ...
  return { response, dynamicUsed, cacheStatus: "MISS" };
}
...
async function _handleRequest(request, __reqCtx, _mwCtx) {
  ...
  if (route.routeHandler) {
    ...
    if (typeof handlerFn === "function") {
      ...
      try {
        const __isrResult = await __handleRouteWithIsrCache({
          pattern: handler.pattern,
          routeKey: __isrRouteKey(cleanPathname),
          revalidateSeconds,
          isProduction: process.env.NODE_ENV === "production",
          isForceDynamic: handler.dynamic === "force-dynamic",
          isGetOrHead: method === "GET" || isAutoHead,
          isAutoHead,
          handlerFn,
          request,
          params,
          cleanPathname,
          url,
          execCtx: _getRequestExecutionContext(),
          createRegenContext: () => _createUnifiedCtx({
            headersContext: { headers: new Headers(), cookies: new Map() },
            executionContext: _getRequestExecutionContext(),
          }),
        });

        // HIT/STALE — return cached response directly
        if (__isrResult.cacheStatus === "HIT" || __isrResult.cacheStatus === "STALE") {
          setHeadersContext(null);
          setNavigationContext(null);
          return attachRouteHandlerMiddlewareContext(__isrResult.response);
        }

        const response = __isrResult.response;
        const dynamicUsedInHandler = __isrResult.dynamicUsed;
